...
{
   const uint32 enableBit = 0x8000000C; //0x80000000UL
...
{
   PCI_ConfigWrite32(addr, offset_of(PCIConfigSpace, BAR[index]), value);
}
...
{
   uint32 bar = PCI_ConfigRead32(addr, offset_of(PCIConfigSpace, BAR[index]));
...
{
   uint16 command = PCI_ConfigRead16(addr, offset_of(PCIConfigSpace, command));
   ...
   PCI_ConfigWrite16(addr, offset_of(PCIConfigSpace, command), command);
}
